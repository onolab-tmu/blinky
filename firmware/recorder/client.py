...
parser = ArgumentParser(description='Clietn for Blinky recorder')
parser.add_argument('-b', '--blinkies', default="", help='Specify blinky mDNS names, like blinky1.local:blinky2.local:blinky3.local')
parser.add_argument('-o', '--output', default="./", help='Specify output folder like. ./output')
...
def download(v):
    ...
    try:
        ...
        mkdir_p(args.output)
        ww = wave.open('%s/%s_%s_%d_%s.wav' % (args.output,address,chmodestr,samplerate,time.strftime("%Y%m%d-%H%M%S")), 'w')
        chs=1
        ...
        while True:
            ...
            while(len(response)>=chunk):
                ...
                if lastidx!=-1 and abs(idx-lastidx)!=1 and abs(idx-lastidx)!=65535:
